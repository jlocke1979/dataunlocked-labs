...
function renderTreemapPanel(g, entries, bbox, { title, panelKind, organ }) {
  ...
  cell
    ...
    .text(d => {
      ...
      return `${d.data.diagnosis}\n${d3.format(",")(d.data.value)} (${share}% of ${organ})`;
    });
...
export function runDiagnosisTreemap(options = {}) {
  ...
  function draw(organ) {
    ...
    renderTreemapPanel(waitPanel, waitEntries, {
      ...
      width: PANEL_WIDTH,
      height: PANEL_HEIGHT
    }, { title: "Waitlist registrations (share within organ)", panelKind: "waitlist", organ });
    ...
    renderTreemapPanel(txpPanel, txpEntries, {
      ...
      width: PANEL_WIDTH,
      height: PANEL_HEIGHT
    }, { title: "2025 transplants (share within organ)", panelKind: "transplant", organ });
