...
export function runWaitlistVsTransplants() {
  ...
  const TITLE_Y = 60;
  const SUBTITLE_Y = 92;
  const BOTTOM_LABEL_Y = height - 46;
  const SOURCE_Y = height - 8;
  ...
  svg.append("text")
    ...
    .attr("fill", "#2f3e34")
    .text("Organ Demand Exceeds Supply Across All Categories");
  ...
  svg.append("text")
    ...
    .attr("font-size", 15)
    .attr("fill", "#6f6a5f")
    .text("Kidney demand dominates; smaller categories show persistent but lower absolute gaps.");
  ...
  svg.append("text")
    ...
    .attr("font-size", 12)
    .attr("fill", "#8a8479")
    .text("Dots are scaled approximations; each dot represents about 500 patients.");
  ...
  const nodes = categories.flatMap((cat, rowIdx) => {
    ...
    const transplantRows = Math.max(1, Math.ceil(tCount / 10));
    const rowOffset = ((waitlistRows - transplantRows) * DOT_STEP) / 2;
  ...
  circles
    ...
    .attr("cx", d => d.x)
    .attr("cy", d => d.y)
    .attr("opacity", d => d.side === "transplants" ? 0.95 : 0.35);
  ...
  svg.selectAll("text.ratio-label")
    .data(categories)
    .enter()
    .append("text")
    ...
    .attr("opacity", 0)
    .text(d => {
      const tCount = transplantCounts[d.organ] || 0;
      const wCount = Math.max(0, d.count - tCount);
      return tCount > 0 ? `${Math.round(wCount / tCount)}:1` : "n/a";
    })
    .transition()
    .delay(250)
    .duration(500)
  ...
  svg.append("text")
    ...
    .attr("font-size", 10)
    .attr("fill", "#8a8479")
    .text("Source: OPTN / UNOS national data reports, 2025");
}