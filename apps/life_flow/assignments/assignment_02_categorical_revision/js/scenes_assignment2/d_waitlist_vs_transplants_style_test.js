...
export function runWaitlistVsTransplantsStyleTest() {
  const width = 1200;
  const height = 675;
  const marginX = 72;
  const rowStartY = 232;
  ...
  const svg = d3.select("#vis")
    ...
    .attr("height", "100vh")
    .attr("role", "img")
    .attr("aria-label", "Life Flow styled waitlist versus transplant example");
  ...
  svg.append("text")
    ...
    .text("The Gap Between Need and Care");
  ...
  svg.append("text")
    ...
    .attr("fill", colors.text)
    .text("Waitlist demand and completed transplants by organ type, 2025");
  ...
  const tiles = rowData.flatMap((row, index) => {
    ...
    return waffleTiles(row.waitlist + row.transplantTiles, waffleX, y, waffleCols).map((point, tileIndex) => ({
      ...point,
