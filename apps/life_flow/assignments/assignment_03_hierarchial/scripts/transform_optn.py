...
# Create treemap-specific summary data for hierarchical visualization
treemap_path = processed_path.parent / "transplants_treemap.csv"

treemap = (
    clean
    .dropna(subset=["donor_type", "organ"])
    .query("organ != 'All Organs' and donor_type != 'All Donor Types'")
    .groupby(["donor_type", "organ"], as_index=False)["transplants"]
    .sum()
)

# Ensure integer counts and sorted descending by total transplants
treemap["transplants"] = treemap["transplants"].astype(int)
treemap = treemap.sort_values("transplants", ascending=False).reset_index(drop=True)

treemap.to_csv(treemap_path, index=False)

print(f"✅ Wrote {len(clean)} rows to {processed_path}")
print(f"✅ Wrote {len(treemap)} rows to {treemap_path}")
